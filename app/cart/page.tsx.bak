...
export default function CartPage() {
  ...
  const [addressServiceable, setAddressServiceable] = useState<boolean | null>(
    null
  );
  ...
  const distanceAddressRef = useRef<string | null>(null);
  ...
  const loadAddresses = useCallback(async () => {
    ...
    const chosen =
      (currentPin &&
        list.find(
          (address) =>
            String(address.pincode || address.pin || "") === currentPin
        )) ||
  ...
  useEffect(() => {
    ...
    if (pin !== pincode) {
      setPincode(
        pin,
        String(selected.area || selected.area_name || "")
      );
    }
    ...
    void fetch(`/api/pincode/${pin}/areas`)
      ...
      .catch(() => {
        if (!cancelled) setAddressServiceable(false);
      });

    return () => {
      cancelled = true;
    };
  }, [selected, isAuthenticated, pincode, setPincode]);

  const cartValidationKey =
    cart.items
  ...
  if (!cart.items.length) {
    return (
      <main className="mk-cart-page mk-cart-empty-state">
        <header className="mk-cart-page-header">
          <button
            type="button"
            onClick={() => router.back()}
            aria-label="Back"
            className="mk-cart-back-button"
          >
            ←
          </button>
          <h1>Your Cart</h1>
        ...
        <section className="mk-cart-empty-page">
          ...
          <span className="mk-cart-eyebrow">YOUR CART</span>
  ...
  async function chooseAddress(address: Address) {
    ...
    const pin = String(address.pincode || address.pin || "")
      .replace(/\D/g, "")
      .slice(0, 6);

    if (pin.length !== 6) {
      setError("This address does not have a valid pincode.");
      return;
    }

    setPincode(
      pin,
      String(address.area || address.area_name || "")
    );
  ...
  async function saveNewAddress(address: Address) {
    setAddresses((current) =>
      address.id != null &&
      current.some((item) => String(item.id) === String(address.id))
        ? current.map((item) =>
            String(item.id) === String(address.id) ? address : item
          )
        : [...current, address]
    );
  ...
  async function placeOrder() {
    ...
    if (!/^\d{6}$/.test(pincode)) {
      setError("Please add a valid delivery address first.");
      return;
    }

    if (!validatedCart) {
      setError("Please wait while your delivery charges are calculated.");
      return;
    }
    ...
    try {
      ...
      const razorpay = new window.Razorpay({
        ...
        handler: (payment: { razorpay_payment_id?: string }) => {
          ...
          if (result.order_no) {
            params.set("order_no", String(result.order_no));
          }
  ...
  return (
    <main className="mk-cart-page">
      <header className="mk-cart-page-header">
        ...
        <div className="mk-cart-page-title">
          <span>Your Cart</span>
        ...
        <Link
          ...
          className="mk-cart-add-button"
        >
          +
        </Link>
      </header>

      <div className="mk-cart-layout mk-cart-app-layout">
        <div className="mk-cart-main-column">
          <div className="mk-cart-address-sticky-wrap">
            <DeliveryAddress
              address={selected}
          ...
          {isAuthenticated &&
            ...
            addressServiceable === false && (
              <div className="mk-address-not-serviceable">
                <span>⚠️</span>
                <div>
                  <strong>We can’t deliver to this address</strong>
                  <small>Choose another saved address to continue.</small>
                </div>
                <button
                  type="button"
          ...
          <section className="mk-cart-items-section">
            <div className="mk-cart-section-heading">
              ...
              <span className="mk-cart-count-pill">
                {itemCount}
              </span>
            </div>
          ...
          {error && (
            <div className="mk-checkout-error" role="alert">
              {error}
            </div>
          )}
        </div>
      ...
      {formOpen && (
        <ModalPortal>
          <div
          className="mk-address-form-screen"
          role="dialog"
          aria-modal="true"
          aria-label="Add new delivery address"
        >
          <div className="mk-address-form-panel">
            <header className="mk-address-form-header">
              <button
                type="button"
              ...
              <span className="mk-address-header-spacer" />
            </header>

            <div className="mk-address-form-scroll">
              <p className="mk-address-form-intro">
                Add your delivery details. We’ll first check the
                pincode and available delivery area.
              </p>
